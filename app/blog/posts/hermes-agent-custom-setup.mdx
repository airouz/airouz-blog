...
summary: 'AI 코딩 에이전트를 실무에서 쓰려면 기본 설정만으로는 부족하다. 모델 라우팅, 스킬 시스템, MCP 연동, Discord 게이트웨이까지, 내가 Hermes Agent를 어떻게 세팅했는지 정리한다.'
---
...
- [SOUL.md: 에이전트 페르소나](#soulmd-에이전트-페르소나)
- [14개 커스텀 페르소나](#14개-커스텀-페르소나)
...
`SOUL.md` 파일은 에이전트의 성격과 행동 원칙을 정의한다. 단순한 시스템 프롬프트가 아니라, 에이전트가 "누구인지"를 정의하는 문서다.
...
이게 왜 중요하냐면, 에이전트의 응답 품질은 성격 정의에 크게 좌우되기 때문이다. 단순히 "도움이 되는 어시스턴트"라고만 하면, 매번 다른 톤으로 응답하고 일관성이 떨어진다. 구체적인 캐릭터를 부여하면 응답의 일관성이 크게 올라간다.
...
`config.yaml`의 `agent.personalities`에 14개의 페르소나를 정의해두었다. 상황에 맞게 전환할 수 있다.
...
총 118개의 스킬을 21개 카테고리에 나눠서 관리한다.
...
- **훈련**: axolotl, PEFT, Unsloth, flash-attention, accelerate, torchtitan, TRL, GRPO
- **추론**: vLLM, llama-cpp, GGUF, TensorRT-LLM, outlines, instructor
- **평가**: lm-evaluation-harness, weights-and-biases, SAELens
- **데이터**: nemo-curator, huggingface-tokenizers
- **모델**: Stable Diffusion, Whisper, CLIP, LLaVA, AudioCraft
...
- **미디어 (4개)**: YouTube 트랜스크립트, GIF 검색, 음악 생성, 오디오 시각화
...
- **Dogfood (1개)**: Hermes Agent 자체 QA 테스팅과 설정 도우미
...
허용 사용자도 특정 Discord ID로 제한해서, 다른 사람이 에이전트를 호출하지 못하게 했다.
...
기본 TTS는 Microsoft Edge TTS를 사용한다. 무료이고 품질이 준수하다. 대안으로 ElevenLabs, OpenAI, NeuTTS도 설정해두었는데, 필요할 때 전환할 수 있다.
...
음성 인식은 로컬 Whisper (base 모델)를 사용한다. 외부 API 없이 로컬에서 동작하므로 비용이 들지 않고, 음성 데이터가 외부로 나가지 않는다.

녹음은 `Ctrl+B`로 시작하고, 최대 120초까지 녹음 가능하다.
...
- **SOUL.md**: 에이전트의 정체성을 명확하게 정의해서 응답 일관성을 확보한다
